...
####+END
** +
** TODO [#A] [[file:/bisos/core/raw/rebuild/AI-WorkPlan.org]]  (aliases, testing, CBM top-level)
SCHEDULED: <2026-08-31 Mon>
Perhaps in combination with [[rawBisosCapabilities.cs]]
** TODO [#A] Create <<rawBisosCapabilities.cs>> -- Or rawBisosCbm.cs
SCHEDULED: <2026-08-21 Fri>
To run separately. After rawBisos-Basic. rawBisos-Cbm.
** TODO [#A] Use Marmee to send out messages to Frank Vanris
** TODO [#A] Sort out domains -- Register soncli.com  -- and matoof.com -- do them through google.
** TODO [#A] In _URGENT_  bro_dockerContainer build.bash is not consistent with .spcs
SCHEDULED: <2026-08-31 Mon>
 Image names don't match cp of raw-bisos is missing  --- Perhaps we can have a imageName/value file
** TODO [#B] Now that there is aiActivity.cs Each pkg and repo has its own WorkPlan. Link to those from here.
SCHEDULED: <2026-08-31 Mon>
** TODO [#A] Create a "Relevant Info" top panel. Categorize by topic.
SCHEDULED: <2026-08-04 Tue>
Example: RelevantInfo/AI/noChatbotPaper   Then from AI, link to Relevant Info.
Same Story with emacs, LaTeX, cheet sheets, etc. Quick References, Interesting Articles.
** TODO [#A] From /bisos/platform and /bisos/site create links to bro_rawBisos
SCHEDULED: <2026-08-26 Wed>
- ln -s /bisos/git/bxRepos/bxObjects/bro_rawBisos/bro_rawBisosPlatform platform
- ln -s /bisos/git/bxRepos/bxObjects/bro_rawBisos/bro_rawBisosSite site
** TODO [#A] Add platformLimits.cs to RawBisos somewhere -- After if when_virtTypeIs == VirtType.
SCHEDULED: <2026-08-23 Sun>
** WAITING [#A] Needs Testing:: nvm and npm and node() to sbom-seed.cs
SCHEDULED: <2026-08-23 Sun>
- Replace exisiting npm usage with npm-sbom.pcs
** TODO [#A] Capture shm increases from within priviledged systemd
SCHEDULED: <2026-08-26 Wed>
** TODO [#A] Add no --sandbox  to google-chrome Startup.
SCHEDULED: <2026-08-26 Wed>
** -B|
* +
* _IMPORTANT_  --- Create /bisos/core/raw/bin   ---- Everything that is needed by rawBisos
* -B|
...
*** DONE @atexit.register module.plant of seeded scripts
*** TODO Clean up exmpl-seeded-cmnds.cs
*** TODO Clean up exmpl-plant-cmnds.cs
*** TODO Use 'plantedCsu' concept in main.py and seedCmnd.cs
*** TODO Add comments to b.imported about __main__.__file__ with atexit
*** TODO Have .setup of seed have precedence.
*** TODO Figure out multi-seed paradigm.
*** TODO Test sbom with a Command and examples
*** -----------
*** TODO First Do it For Facter
*** TODO Use all of above in Fresh VM to Station site registrar
*** TODO Revisit sites reconstruction Using siteBootstrap model of panels/sites
*** TODO in panels/site create 2 sections -- 1) Site Initial Construction -- 2) Platform Site Reconstruction
*** TODO build and use siteBootstrap in bisos.sites and all relevant
*** WAITING [#A] Re-do generic guest build using .153 as registrar
*** TODO [#A] Create a  /dedicated VM/ for registrars. Start from sysCharRealize.sh
*** TODO [#A] Add reg capability to  dedicated VM.
*** TODO Repeat above for NAT, DHCP, WifiBridge
...
** TODO Global Terminology Change For:: bxo becomes bpo (portable) , bro (repo), bso (static) - perhaps bfo (file)
...
####+END
** +
** TODO [#A] bisos.graphviz needs to have a pypi install.
** TODO [#A] bisosSetupLcnt  For rawBisos
SCHEDULED: <2026-05-12 Tue>
- [AtHome] Copy ~pip_lcntBases bxObjects/bro_rawBisos/bro_lcntBases
- [AtHome] Consider creating  bro_lcntExamples
- E: Unable to locate package msttcorefonts
 /bisos/core/bsip/bin/buePersian.sh::[209] -- buePersian.sh -- vis_persianFonts -- root 20260512095109927971627
 [209]: vis_fontPkgsObtain
** TODO Web UI -- csPlayer for rawBisos (Also on fulllContainer)
- bro_rawBisosPlatform/sys/bin Is the strating point . Put cap from bisos.webCap there.
- bxRepos/bisos-web --- Add to common/bases -- csLineInvoker-react  csPlayerPerf-drf
- bxRepos/cliGui
- bisos.webCap
- Container Systemd Replacement
- Add csPlayerWebUi to asc/web graphviz
** TODO [#A] List of common bases
- bisos-web // /bisos/git/bxRepos/bisos-web/  csPlayerPerf-drf csLineInvoker-react
- Add /bisos/git/anon/ext/misc/docker-systemctl-replacement
** TODO [#A]  Add to blee Attach to client
- Running blee should record its pid in /bisos/run/bleePid When it starts server
- Add an attach to examples of blee script
** DONE [#A] Add sbomsList to sbom-seed.py
** TODO setup user environment  ~/bpos/ for bro_rawBisosUser
** TODO  move mbDevUser repo to a different organization. bananMohsen, perhaps.
** TODO F11 n n does not work for rawBisos
** DONE [#A] [AtHome] At "By* is about autonomy oriented redecentralization ..."
** TODO [#A] [Away]  clone panels bisos-web
** TODO [AtHome] For rawBisos  /bisos/var/currents  link to bro_unsitedSite
- Cleanup bisosSiteGitServer.sh  -- It has credentials IMPORTAN
- Add bisosSite* to sites panel
** TODO [AtHome] In /bisos/var/selected  link to bro_unsitedSite
- Missing /bisos/panels/bisos-core/sites/linkToSiteRealmPanels -- Adjust to bro_rawBisosSite
...
*** -B|
** TODO aasMarmeeMaanage.cs
- File "/bisos/pipx/venvs/bisos-marmee/lib/python3.11/site-packages/bisos/bpo/bpoRepo.py", line 96, in <module>
    import pygit2
** TODO [#A] Converge bisos-pip/coreDist  into bisos-pip/sbom -- Include /etc/apt/sources.list contrib needed for tor
** TODO [#A] sudo apt-get install ntpsec  --- Also create a panel for time in platform
...
** Archive :ARCHIVE:
*** DONE /run/user/2001 for bx-dbock -- instead use /tmp/emacsUid//xxpidxx/
:PROPERTIES:
:ARCHIVE_TIME: 2026-05-06 Wed 09:18
:END:
*** DONE apt-get install whois -- added to rawBisos-sbom.pcs
:PROPERTIES:
:ARCHIVE_TIME: 2026-05-06 Wed 10:18
:END:
* /[[elisp:(beginning-of-buffer)][|^]]  [[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]] [[elisp:(delete-other-windows)][|1]]/
